...
/// The RecipeDisplay variants currently generated by Pumpkin's recipe book.
/// Variant ordinals are the 26.2 RECIPE_DISPLAY registry IDs.
#[derive(Clone, Debug)]
...
pub enum SlotDisplay<'a> {
    ...
    AnyFuel,
    Item(&'a Item),
    ItemStack { item: &'a Item, count: u8 },
    Composite(Vec<SlotDisplay<'a>>),
}

/// Dynamic recipes are assigned display IDs only when the recipe book writes them.
/// Brewing recipes are not represented in that display sequence.
fn is_displayable_dynamic_recipe(recipe: &DynamicRecipe) -> bool {
    !matches!(recipe, DynamicRecipe::Brewing(_))
}

pub fn dynamic_recipe_display_count(recipes: &[DynamicRecipe]) -> usize {
    recipes.iter().filter(|recipe| is_displayable_dynamic_recipe(recipe)).count()
}

pub fn dynamic_recipe_for_display_id(
    recipes: &[DynamicRecipe],
    display_index: usize,
) -> Option<&DynamicRecipe> {
    recipes
        .iter()
        .filter(|recipe| is_displayable_dynamic_recipe(recipe))
        .nth(display_index)
}
...
fn write_slot_display(
    write: &mut impl Write,
    display: &SlotDisplay<'_>,
    version: JavaMinecraftVersion,
) -> Result<(), WritingError> {
    match display {
        SlotDisplay::Empty => write.write_var_int(&VarInt(
            remap_slot_display_id_for_version(SLOT_DISPLAY_EMPTY, version) as i32,
        ))?,
        SlotDisplay::AnyFuel => write.write_var_int(&VarInt(
        ...
        SlotDisplay::ItemStack { item, count } => {
        ...
        SlotDisplay::Composite(contents) => {
            ...
            write.write_var_int(&VarInt(contents.len() as i32))?;
            for display in contents {
                write_slot_display(write, display, version)?;
            }
        }
    }
    Ok(())
...
/// Serialize only RecipeDisplay.STREAM_CODEC (registry type + its typed payload).
pub fn write_recipe_display(
    write: &mut impl Write,
    display: &RecipeDisplay<'_>,
    version: JavaMinecraftVersion,
) -> Result<(), WritingError> {
    match display {
        RecipeDisplay::Shapeless { ingredients, result, crafting_station } => {
            write.write_var_int(&VarInt(RECIPE_DISPLAY_SHAPELESS))?;
            write.write_var_int(&VarInt(ingredients.len() as i32))?;
            for ingredient in ingredients {
                write_slot_display(write, ingredient, version)?;
            }
            write_slot_display(write, result, version)?;
            write_slot_display(write, crafting_station, version)?;
        }
        RecipeDisplay::Shaped { width, height, ingredients, result, crafting_station } => {
            if ingredients.len() != (*width as usize).saturating_mul(*height as usize) {
                return Err(WritingError::Message("shaped display dimensions do not match ingredients".into()));
            }
        ...
        RecipeDisplay::Furnace { ingredient, fuel, result, crafting_station, duration, experience } => {
            write.write_var_int(&VarInt(RECIPE_DISPLAY_FURNACE))?;
            write_slot_display(write, ingredient, version)?;
            write_slot_display(write, fuel, version)?;
            write_slot_display(write, result, version)?;
            write_slot_display(write, crafting_station, version)?;
...
fn write_item_slot_display(
    ...
    write_slot_display(write, &SlotDisplay::Item(item), version)
}
...
fn write_item_stack_slot_display(
    ...
    write_slot_display(write, &SlotDisplay::ItemStack { item, count }, version)
}
...
fn write_empty_slot_display(
    ...
    write_slot_display(write, &SlotDisplay::Empty, version)
}
...
fn write_any_fuel_slot_display(
    ...
    write_slot_display(write, &SlotDisplay::AnyFuel, version)
}
...
impl ClientPacket for CRecipeBookAdd<'_> {
    fn write_packet_data(
        ...
        let crafting_count: usize = RECIPES_CRAFTING
            ...
            .count();
        let dynamic_count = dynamic_recipe_display_count(self.dynamic_recipes);
...
mod tests {
    ...
    fn brewing_recipes_are_excluded_from_count_and_display_ids() {
        ...
        let cooking_then_brewing = serialize(&[cooking_recipe(), brewing_recipe()]);
        assert_eq!(brewing_then_cooking, cooking_only);
        assert_eq!(cooking_then_brewing, cooking_only);

        let recipes = [brewing_recipe(), cooking_recipe(), brewing_recipe()];
        assert_eq!(dynamic_recipe_display_count(&recipes), 1);
        assert!(matches!(
            dynamic_recipe_for_display_id(&recipes, 0),
            Some(DynamicRecipe::Cooking(_))
        ));
        assert!(dynamic_recipe_for_display_id(&recipes, 1).is_none());
    }
}
